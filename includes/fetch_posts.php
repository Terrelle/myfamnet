...
$body = preg_replace($url, '<a style="color:CC0033;" href="$0" target="_blank" title="$0">$0</a>', $post_body);
if ($isHashTagPage == "yep") {
    $body = preg_replace($hashtags_url, '<a style="color:CC0033;" href="'.$hashtag_path.'$2" title="#$2" class="hashtagHightlight">#$2</a>', $body);
}else{
    $body = preg_replace($hashtags_url, '<a style="color:CC0033;" href="'.$hashtag_path.'$2" title="#$2">#$2</a>', $body);
}
$body = nl2br("$body");
    echo "
...
if ($isHashTagPage == "yep") {
    ...
    $sh_body = preg_replace($sh_hashtags_url, '<a style="color:#CC0033;" href="'.$sh_hashtag_path.'$2" title="#$2">#$2</a>', $sh_body);
}
...
<div class=\"post_like_comment_share\">
    <a href=\"javascript:void(0);\" onclick=\"fcomment('$get_post_id')\" class='post_like_comment_shareA' data-toggle='tooltip' data-placement='top' title='".lang('comment')."'><span style='font-size: 30px;' class=\"fa fa-comment\"></span></a>";
$s_id = $_SESSION['id'];
...
$c_num = $c->rowCount();
if ($c_num > 0){
    echo "<span id='likeUnlike_$get_post_id' style='cursor:pointer'><span onclick=\"likeUnlike('$get_post_id')\" style='color:#CC0033;font-size:30px' data-toggle='tooltip' data-placement='top' title='".lang('u_liked_this')."' id='punlike'><span class=\"fa fa-heart\"></span></span></span>";
}else{
    echo "<span id='likeUnlike_$get_post_id' style='cursor:pointer'><span onclick=\"likeUnlike('$get_post_id')\" style='color:#CC0033;font-size:30px' data-toggle='tooltip' data-placement='top' title='".lang('liked')."' id='plike'><span class=\"fa fa-heart-o\"></span></span></span>";
}
...
if ($likes_num == 0) {
    ...
    $likenum = "1 <span class='fa fa-heart' style='color: #CC0033;'></span>";
}else{
    $likenum = thousandsCurrencyFormat($likes_num)." <span class='fa fa-heart' style='color: #CC0033;'></span>";
}

echo "<a href='javascript:void(0);' onclick=\"sharePost('$get_post_id','$check_path')\" class='post_like_comment_shareA' data-toggle='tooltip' data-placement='top' title='".lang('share_now')."'><span style='font-size: 30px;' class=\"fa fa-share\"></span></a>
...
<p style='font-size: 10px; padding: 0px 10px; border: none; margin: 0; margin-top: 3px;'></p>
</div>
